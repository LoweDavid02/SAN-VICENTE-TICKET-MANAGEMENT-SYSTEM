...
export default function Landing() {
  ...
  const [animatedStats, setAnimatedStats] = useState({ tickets: 0, resolution: 0, uptime: 0, personnel: 0 });
  ...
  return (
    <div style={{ fontFamily:'var(--font-sans)', background:'#f8fafc', minHeight:'100vh', overflowX:'hidden' }}>
      ...
      <section id="hero" style={{ position:'relative', minHeight:'100vh', overflow:'hidden' }}>
        ...
        {/* Dark overlay */}
        <div style={{ position:'absolute', inset:0, background:'rgba(4,20,28,.85)' }} />
      ...
      <section id="about" ref={aboutRef} style={{ padding:'96px 24px', background:'#fff', opacity: aboutVisible ? 1 : 0, transform: aboutVisible ? 'translateY(0)' : 'translateY(30px)', transition: 'opacity 0.6s ease-out, transform 0.6s ease-out' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:20 }}>
            {[
              { color:'#2563eb', bg:'#eff6ff', border:'#bfdbfe', title:'Guest Submission', desc:'Submit service requests without creating an account. Track your request status anytime using your unique tracking code. Quick, easy, and secure.', delay: 0 },
              { color:'#f59e0b', bg:'#fffbeb', border:'#fde68a', title:'Personnel Portal', desc:'Field staff receive assigned tasks, update ticket statuses from the field, and document completed work from a mobile-friendly interface.', delay: 0.1 },
              { color:'#14b8a6', bg:'#f0fdfa', border:'#99f6e4', title:'Admin Portal', desc:'Administrators oversee all operations, manage personnel, view analytics dashboards, and ensure no request falls through the cracks.', delay: 0.2 },
            ].map((p, idx) => (
              ...
              }} onMouseEnter={(e) => { e.currentTarget.style.transform='translateY(-4px)'; e.currentTarget.style.boxShadow='0 12px 32px rgba(15,23,42,.1)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform=''; e.currentTarget.style.boxShadow=''; }}>
      ...
      {/* FEATURES */}
      <section id="features" ref={featuresRef} style={{ padding:'96px 24px', background:'#f8fafc', opacity: featuresVisible ? 1 : 0, transform: featuresVisible ? 'translateY(0)' : 'translateY(30px)', transition: 'opacity 0.6s ease-out, transform 0.6s ease-out' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:56 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))', gap:16 }}>
            {FEATURES.map((f, idx) => {
              ...
              return (
                ...
                }} onMouseEnter={(e) => { e.currentTarget.style.transform='translateY(-3px)'; e.currentTarget.style.boxShadow='0 8px 24px rgba(15,23,42,.1)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform=''; e.currentTarget.style.boxShadow='0 1px 3px rgba(15,23,42,.05)'; }}>
                  <div style={{ width:44, height:44, borderRadius:12, background:f.bg, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:16, transition:'transform .2s' }} onMouseEnter={(e) => e.currentTarget.style.transform='scale(1.1) rotate(5deg)'} onMouseLeave={(e) => e.currentTarget.style.transform=''}>
      ...
      {/* CONTACT */}
      <section id="contact" ref={contactRef} style={{ padding:'96px 24px', background:'#fff', opacity: contactVisible ? 1 : 0, transform: contactVisible ? 'translateY(0)' : 'translateY(30px)', transition: 'opacity 0.6s ease-out, transform 0.6s ease-out' }}>
        <div style={{ maxWidth:700, margin:'0 auto', textAlign:'center' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:16, marginBottom:40 }}>
            {[
              ...
              <div key={label} style={{ 
                ...
                transition: 'all .2s',
                transitionDelay: `${delay}s`
              }} onMouseEnter={(e) => { e.currentTarget.style.transform='translateY(-4px)'; e.currentTarget.style.boxShadow='0 8px 20px rgba(15,23,42,.08)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform=''; e.currentTarget.style.boxShadow=''; }}>
                <div style={{ width:36, height:36, borderRadius:10, background:'#f0fdfa', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 10px', transition:'transform .2s' }} onMouseEnter={(e) => e.currentTarget.style.transform='scale(1.1)'} onMouseLeave={(e) => e.currentTarget.style.transform=''}>
          ...
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={() => navigate('/submit')} style={{ display:'inline-flex', alignItems:'center', gap:10, padding:'14px 36px', borderRadius:14, border:'none', background:'#22a83a', fontSize:15, fontWeight:700, color:'#ffffff', cursor:'pointer', fontFamily:'inherit', boxShadow:'0 2px 12px rgba(34,168,58,.25)', transition:'all .2s' }} onMouseEnter={(e) => { e.currentTarget.style.transform='translateY(-2px)'; e.currentTarget.style.boxShadow='0 4px 16px rgba(34,168,58,.35)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform=''; e.currentTarget.style.boxShadow='0 2px 12px rgba(34,168,58,.25)'; }}>
              <span style={{ pointerEvents:'none', color:'#ffffff' }}>Submit Request</span>
              <ArrowRight size={16} style={{ pointerEvents:'none', color:'#ffffff' }} />
            </button>
            <button onClick={() => navigate('/login')} style={{ display:'inline-flex', alignItems:'center', gap:10, padding:'14px 36px', borderRadius:14, border:'1.5px solid #e2e8f0', background:'#fff', fontSize:15, fontWeight:600, color:'#475569', cursor:'pointer', fontFamily:'inherit', transition:'all .2s' }} onMouseEnter={(e) => { e.currentTarget.style.transform='translateY(-2px)'; e.currentTarget.style.borderColor='#cbd5e1'; e.currentTarget.style.boxShadow='0 4px 16px rgba(15,23,42,.08)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform=''; e.currentTarget.style.borderColor='#e2e8f0'; e.currentTarget.style.boxShadow=''; }}>
              Staff Login
            </button>
          </div>
        </div>
      ...
      {/* SCROLL TO TOP */}
      {showTop && (
        <button onClick={scrollTop} style={{ position:'fixed', bottom:28, right:28, zIndex:300, width:44, height:44, borderRadius:'50%', border:'none', background:'#0d9488', color:'#fff', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 2px 12px rgba(13,148,136,.3)', transition:'all .2s', animation:'fadeIn .3s ease-out both' }} onMouseEnter={(e) => { e.currentTarget.style.transform='translateY(-2px) scale(1.05)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform=''; }} aria-label="Scroll to top">
        ...
        <div onClick={() => setShowLearn(false)} style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(9,18,32,.75)', backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)', display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem', animation:'fadeIn .2s ease-out both' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width:'100%', maxWidth:680, background:'#fff', borderRadius:24, overflow:'hidden', boxShadow:'0 4px 8px rgba(15,23,42,.06), 0 32px 72px rgba(15,23,42,.22)', animation:'scaleIn .25s cubic-bezier(.34,1.56,.64,1) both' }}>
            ...
            <div style={{ padding:'40px 40px 32px', minHeight:280, display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center' }}>
              <div style={{ width:72, height:72, borderRadius:20, background:'#f0fdfa', border:'1px solid #99f6e4', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:20 }}>
              ...
              <div style={{ display:'inline-flex', flexDirection:'column', alignItems:'center', padding:'14px 28px', borderRadius:14, background:'#f0fdfa', border:'1px solid #99f6e4' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 24px', borderTop:'1px solid #f1f5f9', background:'#f8fafc' }}>
              ...
              {slide < SLIDES.length - 1 ? (
                ...
                <button onClick={() => { setShowLearn(false); navigate('/login'); }} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:10, border:'none', background:'#0d9488', fontSize:13, fontWeight:700, color:'#fff', cursor:'pointer', fontFamily:'inherit', boxShadow:'0 2px 8px rgba(13,148,136,.2)', transition:'all .15s' }} onMouseEnter={(e) => { e.currentTarget.style.transform='translateY(-1px)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform=''; }}>
